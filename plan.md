...
Browsing an ordinary untracked directory is shallow and demand-driven. Recursive discovery, indexing, type generation, and filesystem watching begin only when a folder is tracked or synced. Account-only operations such as `connect` open arbord's control authority without attaching a filesystem session.

...
- Serve ordinary-file bytes at their logical routes, dispatched by node kind (revision `ETag`, HTTP range, `?raw` override for dual-surface kinds), so native preview/open never requires a direct filesystem reach-through. This replaces the bespoke `GET /Assets/…` static route outright — assets are ordinary files at ordinary logical paths — and there is no separate `/v1/file` query route. In-app rendering resolves asset embeds through the shared logical resolver exactly as it already resolves link clicks; `arbor bake` performs the same resolution at publication time. When visiting lands, reads gain a tree dimension and a visited tree's bytes arrive by proxying the wire's content-addressed objects — a read-only slice of Milestone 7 pulled forward, per the parity-reads section of [spec/arbord-rest.md](spec/arbord-rest.md).

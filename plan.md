...
- Use BlockNote's standard Markdown conversion at the browser boundary for inline strong, emphasis, strikethrough, code, links, and hard breaks. Compare the resulting inline structure with the originally parsed structure so untouched blocks retain their exact source spelling while an edited block is normalized independently. Limit the formatting toolbar/schema to Markdown-representable styles, and retain BlockNote's standard Markdown-aware copy/paste formats.

...
| One access store (schema 20): a tree an account activated or hosts is that account's, and its owner's resource rules alone govern it; `access` keeps only unowned trees' entries; `trees.updated_at`, the reservation status, `account_challenges.claim_digest` and `meta.community_name` are gone | implemented, not deployed: [migration 019](packages/canopyd/migrations/019-one-access-store/README.md) is ready to rehearse | [host](docs/architecture/canopyd/README.md#accounts-and-canonical-paths) |

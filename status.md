...
- **Client state machines.** The document admission machine (web editor, native editor) and the direct Canopy synchronization machine (Arbor Sync, native replica) are pure reducers in both languages that execute one shared fixture. A burst of edits is one admission and normally one accepted update; at most one request is in flight per document session and per tree, with one retained successor. Both adoptions are verified: the unit, self-sync, and protocol suites in TypeScript, the six Swift package suites on macOS, and an app smoke test on macOS and the iOS simulator.

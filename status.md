...
- **Consolidated Wire target on main.** The portable specification and paired TS/Swift target models define material references, atomic resolution declarations, ordinary reconciliation with optional exact-state guards, simplified acceptance outcomes, predecessor identity chains and paged inspection without fixed decision-count caps. The specification requires retained rule evidence with open per-rule schemas, without a dedicated evidence endpoint. Shared vectors test grammar and identity; semantic scenarios remain implementation targets. Accepted-state/read codecs remain staged; request-side adoption is described below. Installed builds are unchanged. The source-edit engine and superseded bounded-inspection implementation remain on `codex/source-intent-experiment`. Follow the [contract and adoption boundary](docs/update-wire-contract.md) and [Plan 011](plans/reliability/011-compatible-accepted-ambiguity.md).

- **Wire format stabilization.** Raw file objects, explicit file/directory/tree entries, sparse bootstrap without a file map, and optional accepted-conflict metadata are implemented in TypeScript and Swift. Migration 005 has rewritten the live trees and reset history after synchronized-client checks and two production-copy rehearsals. Mac and iPhone are upgraded; only the soak remains. Conflict exploration/resolution and the conflict backend remain deferred extensions; Canopy does not yet accept unresolved alternatives.

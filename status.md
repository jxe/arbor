...
| Device keys and sessions (Security 006 Phases 2 and 3, schema 23): a `devices.yaml` entry may carry `key` (`ed25519:` or `p256:`); a key device signs a host challenge for a session token of at most an hour; a digest device moves to a key once, and its credential stops working in the same commit; pairing and claims accept a key; a profile-key reset waits 72 hours, any administrator device may cancel it, and then only the new key device is left. TypeScript protocol and canopyd with shared vectors (`device-keys.json`); the Swift `Overstory` models, signing bytes and client calls pass the same vectors. Clients (Phase 3): Arbor Sync holds an Ed25519 key and hands local clients sessions through `GET /v1/credential`; the iPhone holds a Secure Enclave P-256 key; new claims and pairings use keys, `arbor device move-to-key` and both apps move an existing device, identity backups are passphrase-encrypted (version 2; version 1 still restores), and `arbor me reset` and both apps request, show, cancel and finish resets; client suites and the protocol gate pass. As a home host canopyd also publishes each profile's key devices at `GET /.arbor/profiles/{ProfileTreeID}/device-keys` (accounts §5.4, Security 007's home role) | host deployed and verified 2026-09-26 at schema 23 by [migration 023](packages/canopyd/migrations/023-device-keys/README.md) (build `0621789b`); client code on branch `claude/gallant-planck-mbo862`, not installed; the hands-on end-to-end is still to run and no device of Joe's has moved to a key | [accounts §5](docs/overstory-spec/04-accounts-and-devices.md#5-device-pairing), [Security 006](plans/security/006-device-keys.md) |
...
- **Spec** (phase 1): [accounts](docs/overstory-spec/04-accounts-and-devices.md)
  ...
  `;arbor-config`, who may edit it, its invariants and merge, and declare /
...
- **canopyd** (phase 3): schema 22 (`tree_policy`, `tree_admins`,
  ...
  `POST /.arbor/trees/{T};arbor-config/updates`; execution grants name a
  lender (`execution-authority.ts`, `access.ts`); `ARBOR_RESET_ACCOUNT`
  rewrites `devices.yaml` as an accepted update.
- **Clients** (phase 4): the TypeScript client, Arbor Sync and the CLI claim
...
packages/canopyd/migrations/022-tree-configurations` 5 pass;
...
`/v1/placements/resume`, `arbor pause` and `arbor resume`, a durable
...
/v1/pending` and `arbor pending [--json]` show the exact request the next
...
`@overstory/protocol`. Evidence: `tests/integration/folder-pause.test.ts`

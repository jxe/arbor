...
| Sidecar states in memory: the reference sidecar keeps each engine state decoded and frozen, sharing unchanged records and nodes with the state it came from and identified by a digest of its content; the chunked state encodings and lazy history loading are gone. The snapshot tree merge is its own package, `@overstory/tree-merge`, which Arbor Sync tree recovery now declares | implemented, not deployed | [merge sidecar](docs/architecture/canopyd/merge-tool.md#retained-state) |

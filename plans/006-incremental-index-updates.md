...
- **Risk**: MED — the FTS5 table is an external-content table, so index rows
  ...
  from `files`.
- **Depends on**: none (but see the note about `plans/001-*` under Scope)
- **Category**: perf
...
- `tests/unit/indexer.test.ts` (create, or extend if `plans/001-*` created it)
- `tests/performance/indexer.bench.ts` (add a move/delete measurement — optional, see step 5)

**Out of scope** (do NOT touch):

- The `search()` method and the excerpt shape. If `plans/001-escape-search-excerpts.md`
...
Add tests to `tests/unit/indexer.test.ts` (create it if `plans/001-*` did not).
...
- `plans/001-escape-search-excerpts.md` is mid-flight and has uncommitted

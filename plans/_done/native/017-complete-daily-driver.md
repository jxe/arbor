...
>
> **Drift check**: `git diff --stat dc34126..HEAD -- native packages/client packages/arbord docs/client.md docs plans/native tests`
...
Filesystem presentation follow-up (implemented 2026-08-26): when macOS browses an ordinary directory outside every managed tree, display arbord's fabricated `_index.md` source in the normal editor without materializing anything on open. The session remains absolute-path-addressed and must not call `ensureDocumentIdentity` or mint a `PageID`; the first authored edit materializes a real `_index.md`, after which arbord reports a stored directory body. Managed-tree implicit documents retain their existing lazy identity promotion. Cover implicit path documents, stored path documents, and managed-tree documents independently.

...
- **Shared credential, independent state.** The app uses the daemon's device credential over loopback but installs a sparse snapshot rooted at the accepted Canopy root. The daemon's mutable folder head, pending request, conflict, and availability state belong only to the folder client; they neither seed nor block the app. Concurrent requests reconcile through Canopy.
...
3. **UpdateCoordinator.** Sparse candidates, durable `UpdateHead`, recovery that pulls current for a re-seeded tree, submission hold, and `CanopyWatchRunner`; TS reducer rename to `reduceUpdate`. Every working tree owns only its local heads, attempts, and conflicts. Specs: `spec/09` requires accepted-root installation and independent clients, durable objects with heads and requests, rules 1, 3, 8, 10; fixture key rename and plural-request scenarios; `wire-update-intent.json` envelope-independence vector.
...
- Dirty daemon: with the app already bootstrapped from an accepted root, stop Canopy and edit both on disk and in the app. The app retains and later submits only its own edit; the daemon retains and submits only the folder edit; neither client blocks the other and Canopy reconciles both when connectivity returns.
- Force a daemon conflict: the app stays writable and can publish its own independent work. The folder conflict remains reviewable in Arbor Sync status and does not become the app's conflict.
...
- Both reducers pass the renamed fixture with plural-request recovery scenarios, accepted-root installation, and no filesystem role or cross-client request adoption.

...
- On macOS, native Arbor uses arborsync as the sole first-party writer for ordinary folders and placed trees. The directly signed desktop app is intentionally not App Sandbox-confined: it attaches to the one per-user arborsync and registers its relocatable bundled LaunchAgent with `SMAppService` when no compatible user process or CLI-owned agent exists, so one process owns `~/.arbor` and absolute filesystem navigation. `arbor daemon ...` is the platform-neutral control surface; macOS CLI-only installs use the same launchd label. Launch restores the saved folder, migrates the former sandbox preference bookmark once when necessary, and otherwise opens the user's real home directory; “no provider” is never a steady production state. Test helpers and explicit `ARBOR_DATA_HOME` runs remain isolated foreground processes.
...
## Hunch rehearsal history and completed cutover
...
Plan 019 used repeatable, copy-only rehearsal tooling for the former Hunch workspace. Arbor never opened the source folder for writing, and edits made while evaluating one rehearsal tree could not flow back into retained Hunch data or silently seed a later rehearsal. A stable private conversion recipe preserved the same reviewed PageIDs across runs. This was focused operator tooling, not an app import feature.

The disposable repository-local converter inventoried without writes, drafted a private recipe, required two identical dry-run confirmations, staged and verified every output byte before publishing a new destination, and verified the untouched import baseline afterward. No personal recipe, manifest, content, or hash was checked in. The first private rehearsal was created and verified on 2026-08-25, then opened repeatedly in signed native Arbor builds while the Hunch source remained unchanged. Joe confirmed on 2026-09-04 that no further rehearsals were needed, so the converter, tests, command, and operator documentation were retired; Git history preserves them if a new one-off conversion is ever explicitly requested.

...
> **Executor instructions**: This is the first plan allowed to mutate the configured live authority. Obtain explicit operator confirmation immediately before the live deploy. Never print credential values. Upgrade only after a complete volume backup has been restored and migrated successfully in isolation and the exact committed revision has passed Hetzner. Treat the startup backfill as a temporary migration-release shim: after the live volume passes exact verification, remove the obsolete compatibility paths and deploy the clean runtime. On any live mismatch, stop the new image and restore the backup with the previous image.
>
> **Drift check**: `git diff --stat dc34126..HEAD -- packages/authority packages/wire packages/arbord packages/cli deploy tools tests plans/native`
...
5. Only after the restored-copy rehearsal and Hetzner gate pass, request explicit approval to mutate Railway. Pause writes, confirm the backup is retained, and deploy the exact tested `updates-v1`/private-history/device-capable migration release. Its normal startup performs the transactionally rehearsed one-way volume upgrade before health succeeds.
6. Verify every preflight root is still the current ref, public HTML/Markdown hashes match, canonical discovery/access work, public accepted-history reads are absent, non-current/draft object access is denied to every wire subject, and the retained local credential authenticates with the same account/profile identity. Keep arbord stopped during this verification.
7. Remove the accepted-update/device backfill and obsolete legacy schema mutations from authority startup. Retain current-schema creation for new authorities and add an explicit current-schema assertion so an unmigrated authority fails clearly rather than being modified implicitly. Run the repository gates, push this clean runtime, restart it once, and repeat the exact production checks. The migration shim may remain in Git history but not in the maintained runtime.
8. Create an isolated private Railway test tree; repeat the short production smoke for one-sided sync, additive Markdown merge, structured client-owned conflict, canonical semantic-request replay, watch/internal history, and device revoke. This is a production-environment verification, not a substitute for the earlier Hetzner acceptance gate.
9. If any data verification fails, stop the candidate image, restore the complete pre-upgrade volume backup, redeploy the previous image, and repeat the preflight checks before resuming writes. A cleanup-release code failure may return to the verified migration release without restoring data only after the upgraded volume has passed all exact checks. If verification succeeds, resume writes and retain the backup and previous image until Plan 019 is complete.

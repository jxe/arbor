# Speed 001: Stop rebuilding the whole index on every move or delete
...
- **Risk**: MED — the FTS5 table is an external-content table, so index rows
  ...
  from `files`.
- **Depends on**: none (but see the note about `plans/security/001-*` under Scope)
- **Category**: perf
...
- `tests/unit/indexer.test.ts` (create, or extend if `plans/security/001-*` created it)
...
Add tests to `tests/unit/indexer.test.ts` (create it if `plans/security/001-*` did not).

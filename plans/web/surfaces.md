...
Reference: `native/ArborApp/*.swift` and `native/Packages/ArborQuagmire` at the commit the plan was written (2026-09-19). Quote the native strings when building; parity is in the vocabulary as much as the layout.
...
Backed by BlockNote plus `@arbor/editor`; each item names its project.
...
- Identity: `Profile TreeID` (monospaced, selectable), `Copy Profile TreeID`, `Send this public ID to the Canopy administrator before claiming your account.`, URL field and `Claim Account` — local host only, with an existing identity; identity creation stays native/CLI.
...
Every picker list (sidebar, Search Contents, Move to, Move Page) takes ↑ ↓ ↩ and Escape; block-menu one-key chips while open: `t * 1 [ > # 2 3 p - m c [ ]`. Publish the table in `docs/client.md` in the phase that implements each surface.

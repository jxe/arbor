...
id: pmfd95
---
# Arbor web editor follow-up

Historical identifier: **Smaller project 005**. The filename number is preserved; this plan now belongs to web.

Status: BACKLOG; browser work waits for [Web 023](023-rebuild-the-web-editor-on-the-working-tree.md).
Revalidate the old filesystem transaction references against the new client before execution.
...
5. **Search and picker keyboard behavior and ranking — Medium.** After [Web 008](008-web-native-interface-parity.md) owns the shared search surface and result presentation, add robust ranking and editor-picker behavior beyond that shell: path/title/content weighting, recent-page bias, and picker-specific empty/error states.
...
7. **Link healing UI and orphan indicators — Medium–Hard.** Web 008 owns ordinary bottom-of-page backlink presentation. Here, surface durable-ID healing instead of making it invisible, distinguish stale paths from missing identities, and provide useful orphan diagnostics without forcing eager rewrites.
8. **Emoji completion and editor identity — Medium.** Web 008 owns leading-emoji icons in browser navigation surfaces. Here, retain the icon through editor rename behavior and add keyboard-reliable `:emoji` completion.
...
10. **Persistent hierarchical sidebar, then sidebar drag/drop — Hard.** Web 008 first cleans up the existing contextual sidebar. After that, replace its flat listing with lazy hierarchy while preserving expansion and selection state across navigation; only then reuse the filesystem mutation API for sidebar moves. Do not fork a second mutation or selection model.

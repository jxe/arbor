...
- **Smaller projects** — Bounded product or model outcomes that do not yet justify a major project directory. Promote a cluster only when several related plans share one durable outcome; do not create a one-file top-level directory merely to avoid this grouping.
  ...
  - [Smaller project 010 — Trim the Local Arbor REST surface to its real callers](smaller-projects/010-trim-arborsync-routes.md) — **P2 · PLANNED; do before Smaller project 009.** Arbor Sync only runs on the same machine as its callers: the CLI requires a running Arbor Sync and does drawback-free work through a layer extracted from the daemon; dead, test-only, and duplicate routes go.

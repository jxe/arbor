...
- **Reliability** — Behavior that can become wrong or lose durability under concurrency, recovery, lifecycle changes, or unusual input.
  ...
  - [Reliability 004 — Resolve Canopy conflicts at their authored locations](reliability/004-contextual-canopy-conflict-resolution.md) — **P1 · IN PROGRESS; native per-path conflict review with Current/Mine/Both/Edit choices is implemented.** Remaining: ordered suffix replay, inline hunk placement and the Quagmire accessory seam, and Step 6 crash hardening.
  - [Reliability 006 — Preview and resume initial replica bootstrap](reliability/006-progressive-replica-bootstrap.md) — **P1 · PLANNED; coordinates its validated install handoff with completed Reliability 005.** Show a verified read-only root page early, resume immutable snapshot bytes across interruptions, atomically install the complete replica, then enter normal direct synchronization.
...
- **Smaller cleanup candidates and shared seams**
  - **Split `ArborService` responsibilities** — **Simplification · PARTLY CHARACTERIZED.** The Canopy/Wire orchestration seam is `accountWireClient` in `@arbor/canopy-client` and the durable-configuration seam is the `@arbor/stores` projection (Smaller project 010); the protocol-routing and virtual-system-projection seams remain uncharacterized. Promote when a current change needs one of those independently.
...
- **Smaller projects** — Bounded product or model outcomes that do not yet justify a major project directory. Promote a cluster only when several related plans share one durable outcome; do not create a one-file top-level directory merely to avoid this grouping.
  ...
  - [Smaller project 009 — Shape the Local Arbor REST API around document admission](smaller-projects/009-admission-shaped-rest-api.md) — **P2 · PLANNED; Reliability 005 and Smaller project 010 are done.** Give editors one open/admit/observe/close route set so neither host composes digests, bases, or transports client-side.
  - [Smaller project 008 — Bring Arbor web to native interface parity](smaller-projects/008-web-native-interface-parity.md) — **P2 · PLANNED; coordinates safe search excerpts with Security 001 and reuses the completed Reliability 005 synchronization vocabulary.** Port the native shell, search, sidebar, Share, Accounts, and Sync Status information architecture into platform-appropriate browser UI.
...
- **Further reliability hardening**
  - **Explicit web-editor unload drain** — **READY.** App-controlled navigation already awaits the admission machine's flush; browser `beforeunload`/`pagehide` has no bounded drain and no visible pending state, which Reliability 005 left as a documented limitation. Add one or surface the limitation in the UI.
  ...
  - **Background synchronization versus local mutation** — **REVERIFY.** The direct Canopy machine (completed Reliability 005) distinguishes availability from terminal validation failures; add a focused test that materialization and publication cannot observe a torn local transaction across the daemon's workspace I/O boundary.

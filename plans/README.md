...
- **Reliability** — Behavior that can become wrong or lose durability under concurrency, recovery, lifecycle changes, or unusual input.
  ...
  - [Reliability 005 — Standardize Arbor Sync and direct Canopy client state machines](reliability/005-client-synchronization-state-machines.md) — **P1 · IN PROGRESS; TypeScript adoption verified, Swift adoption awaiting its macOS test run.** Two cross-language state machines are frozen in one shared fixture, run in both reference clients, and documented in `docs/client-state-machines.md` and `spec/09-client-synchronization.md`.
  - [Reliability 006 — Preview and resume initial replica bootstrap](reliability/006-progressive-replica-bootstrap.md) — **P1 · PLANNED; coordinates its validated install handoff with Reliability 005.** Show a verified read-only root page early, resume immutable snapshot bytes across interruptions, atomically install the complete replica, then enter normal direct synchronization.
...
- **Compatibility removal**
  ...
  - [Cleanup 004 — Carve the four thick client packages](cleanups/004-client-package-reorganization.md) — **P1 · CARVED; awaiting macOS verification.** Rename and re-carve the TypeScript and Swift client code into `@arbor/arborsync-client`, `@arbor/canopy-client`, `ArborSyncClient`, and `CanopyClient`, each thick enough to host its Reliability 005 state machine.
...
- **Smaller projects** — Bounded product or model outcomes that do not yet justify a major project directory. Promote a cluster only when several related plans share one durable outcome; do not create a one-file top-level directory merely to avoid this grouping.
  ...
  - [Smaller project 009 — Shape the Local Arbor REST API around document admission](smaller-projects/009-admission-shaped-rest-api.md) — **P2 · PLANNED; after Reliability 005 runs on both platforms.** Give editors one open/admit/observe/close route set so neither host composes digests, bases, or transports client-side.
  - [Smaller project 008 — Bring Arbor web to native interface parity](smaller-projects/008-web-native-interface-parity.md) — **P2 · PLANNED; coordinates safe search excerpts with Security 001 and synchronization meaning with Reliability 005.** Port the native shell, search, sidebar, Share, Accounts, and Sync Status information architecture into platform-appropriate browser UI.

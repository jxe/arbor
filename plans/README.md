...
| One update machine for every working tree | Editors and folders append local changes (with or without intent) to one change log that one `UpdateMachine` runner publishes; delete the admission machine and the daemon's own loop, Swift first | Clients [001](clients/001-reconcile-client-state-machines.md) |
...
| Make merge rules replaceable | Built, not deployed: rehearse and run migration 018, measure cold rebuilds and replay on a production copy, then a per-file sidecar cache and 1,000-file latency | canopyd [016](canopyd/016-sidecar-on-objects-and-history.md) |
| Bound storage and improve slow paths | Measure retained storage and worker costs before packing; progressive placement and targeted performance work | canopyd [001](canopyd/001-pack-object-storage.md), Native [006](swift/006-progressive-replica-bootstrap.md), [performance candidates](catalog.md#hardening-efficiency-polish-etc) |

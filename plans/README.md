...
already recorded in `technical-debt.md` or `editor-todo.md`, which were read
...
- **`blocks.tsx` round-trip has no unit test.** 839 lines of Markdown↔BlockNote
  ...
  annotations, covered only by two browser tests. `editor-todo.md:3` plans to
...
- **No root `CLAUDE.md`/`AGENTS.md`**, despite strong agent-directed conventions
  scattered across `plan.md`, `README.md`, and the doc-ownership split. An agent
...
- **`plan.md:48` claims a `claim` CLI command that does not exist**, and its own
  ...
  listed as remaining work. `plan.md:6` declares itself the single source of
...
- **Surface the broken-link data the indexer already collects.**
  ...
  the inbound direction is exposed (`/v1/backlinks`). `editor-todo.md:7` asks
...
- **Everything in `technical-debt.md` and `editor-todo.md`** — read first and
...
- **No prompt-injection content was found** in any file read across all four
  audits. Every instruction-shaped passage (`plan.md:14`'s "Future agents must
  inspect source…", `technical-debt.md`) is ordinary maintainer documentation

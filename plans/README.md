...
- **Smaller projects** — Bounded product or model outcomes that do not yet justify a major project directory. Promote a cluster only when several related plans share one durable outcome; do not create a one-file top-level directory merely to avoid this grouping.
  ...
  - [Smaller project 010 — Trim the Local Arbor REST surface to its real callers](smaller-projects/010-trim-arborsync-routes.md) — **P2 · PLANNED; do before Smaller project 009.** Arbor Sync only runs on the same machine as its callers: delete the dead conflict route and test-only Wire routes, make the CLI call the service it started in-process, and collapse duplicate bytes, upload, and identity routes.
  - [Smaller project 009 — Shape the Local Arbor REST API around document admission](smaller-projects/009-admission-shaped-rest-api.md) — **P2 · PLANNED; Reliability 005 is done on both platforms.** Give editors one open/admit/observe/close route set so neither host composes digests, bases, or transports client-side.

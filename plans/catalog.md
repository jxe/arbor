...
- [canopyd 013 — Entry metadata and the document-version index](canopyd/013-entry-metadata.md) — **SERVER DEPLOYED 2026-09-22; Arbor Sync and client steps remain.** Directly placed trees (always on iPhone) have no page dates; canopyd keeps a per-entry side table of last-accepted change times outside the hashes, served by a new `entry-metadata` read, plus canopyd 007's `document_versions` index, both filled by one change walk and backfilled by one migration (014).

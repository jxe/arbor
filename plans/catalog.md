...
- [canopyd 005 — Configure each hosted tree, profiles included, in its own configuration tree](soon/005-tree-configuration-trees.md) — **SOON · P3 · PHASES 1–4 LANDED; cutover pending Joe.** Each hosted tree's address, access rules and administrators live in a private per-tree configuration tree, the account configuration is folded into the profile's (devices and `apps.yaml`), and code sponsors are replaced with lending by the named subject. Remaining: run migration 022 and install the clients, together.

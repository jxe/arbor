...
- [canopyd 016 — A merge sidecar built on objects and history](canopyd/016-sidecar-on-objects-and-history.md) — **P2 · PLANNED; after canopyd 015's migration.** Two canopyd-managed APIs (objects and the accepted history log) and one merge question; sidecar state becomes a rebuildable cache, canopyd stores none, and plain edits on the head skip the sidecar.

...
- [canopyd 014 — The merge handles many cases brilliantly](canopyd/014-merge-handles-many-cases.md) — **P3 · IDEAS AND CANDIDATES.** The first transfer round (Markdown list/table/link transfers, same-anchor ordering, keyed JSON/YAML moves, TS/JS function moves) is implemented, not deployed. A menu of 30 candidates: measure what reaches review first, then anchor-adjacent edits, cross-document links, new keys and set-like arrays in structured data, declaration moves in more languages, imports, new formats (BibTeX, notebooks, LaTeX, iCalendar) and suggested resolutions. Open: bump the rule revisions the first round changed.

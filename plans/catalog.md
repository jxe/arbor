...
- [Native 011 — Unify Mac account management with iOS](soon/011-unify-mac-accounts-and-fold-daemon-clients.md) — **SOON · NEEDS DECISION.** Both daemon clients are folded into their callers and the pairing-offer and forget routes are gone; the Mac's claim, pair and identity routes remain because the daemon and iOS keep credentials in different stores. Pick the owner (data home or app) before removing them.
...
- [canopyd 014 — Merge the remaining moved and copied text forms](soon/014-merge-moved-text.md) — **SOON · P3 · PLANNED.** Markdown list/table/link transfers, same-anchor ordering, keyed JSON/YAML moves and TS/JS function declaration moves are implemented (not deployed); what remains is Swift/Python declaration moves, structured moves between files, cross-document fragment and reference links, richer Markdown hosts, and anchor-adjacent edits.

...
- [Security 006 — Device keys alongside credential digests, and recovery](security/006-device-keys.md) — **P3 · DESIGNED; ready for Phase 1, shared with Security 007.** Devices may hold a signing key and open short-lived sessions instead of sending a bearer credential, moving over without a forced migration, and the profile key can reset a lost device list after a wait any administrator device can cancel; one host only.
- [Security 007 — Place trees on other hosts](security/007-placement-hosts.md) — **P3 · DESIGNED; Phase 1 shared with Security 006, the rest after it.** A placement host accepts a profile's key devices from the device keys its home host publishes, cached briefly, so one profile holds canonical trees on several hosts with no per-account data; code there uses only `everyone` and `app` rules until Security 008 brings `apps.yaml` across hosts.

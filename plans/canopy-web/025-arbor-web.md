...
> **Executor instructions**: Build Canopy for the web as one browser bundle that is the TypeScript twin of the native app: the same working tree, change log and update machine as the Mac app ([the update machine](../../docs/implementing-sync-services/update-machine.md)), the same information architecture and vocabulary, served by two hosts. The **local host** is Arbor Sync on loopback, opened by `arbor open`. The **canopyd host** is a host serving the bundle at a tree's canonical URL for a browser that holds a paired device credential. Nobody edits through the daemon; the browser publishes to canopyd itself. Port surfaces, not chrome: no menu bar, sheets, gestures, audio or camera. Clean breaks over shims. Nothing live (Railway, `~/.arbor`, the iPhone) flips without Joe's go-ahead.
>
...
- **Depends on**: historical Native 022 (implemented and live); Native 022 soak closeout in [release and soak](../release-and-soak.md)
...
| Bootstrap | `GET /v1/bootstrap?tree=` (exists) | Built in the browser: the tree's ref route gives the accepted root, and `HostObjectStore` walks directory objects and `.md` entries from it through the existing object route. Same `Bootstrap` shape as the daemon's, produced client-side; no modification dates |
...
| Accounts, devices, share, app permissions | The same on both hosts: the browser opens the profile's tree configuration as an ordinary working tree (bootstrap by its derived TreeID) and edits `devices.yaml`, `mounts.yaml` and `apps.yaml`, and other trees' `access.yaml` through `;arbor-config`, exactly as `CanopyAppModel` does; pairing offers and claims go to canopyd's existing pairing routes with the bearer. `GET /v1/accounts` (exists) only lists which configuration trees the local host knows. The Web 008 idea of an account-scoped daemon REST boundary is dropped | |
...
**Reduced daemon surface (Native 011).** `LocalHost` targets the daemon's
reduced loopback surface in [the Arbor Sync REST API](../../docs/implementing-sync-services/arborsync-api.md)
and writes its own browser-safe client against it: the TypeScript daemon client
is now CLI code (`packages/cli/src/daemon-client.ts`) and the
`@overstory/arborsync-client` package is deleted, so `canopy-web`'s stale
imports of it (already failing) are replaced here, not restored. The daemon no
longer creates pairing offers (`POST /v1/bootstrap/pairings`) and
`POST /v1/local/forget` is gone; a pairing offer is `POST /.arbor/pairings` on
canopyd with the account bearer, which is this plan's model already.
...
|---|---|---|
| `CanopyWorkingTree`, `OverstoryObjectStore` | `@overstory/working-tree`, `@overstory/object-store` (browser-safe core; node stores under `./node`) | state, writes, `UpdateMachine`, `UpdateCoordinator`, `ObjectOverlay`, `LayeredObjectStore`, `DaemonObjectStore`, `HostObjectStore` |
| `CanopyWorkspaceState` | `WorkspaceState` | launch phase, opened tree, trees list, accounts, sharing, credential, the coordinator |
| `CanopyAppModel` | `AppModel` | location, history, sidebar order and search, full-text search, backlinks, editor leases, title-rename proposals, trash prompts, History |
| `CanopyEditorHost` | `EditorHost` | document lookup/creation, mentions, assets, move/relocate, `persistCommit` into `EditorSource` |
| `CanopyConflictReviewModel` | `ConflictReviewModel` | choices, drafts, preview, apply |

These are plain TypeScript classes over a small observable store; React components only render them. Every model has tests without React, the `working-tree-updates` fixture runs unchanged from `docs/overstory-spec/conformance/client-state-machines.json`, and the runner vectors in `tests/fixtures/update-runner.json` run against the browser ports.
...
**Phase 0 — bookkeeping.** This file and [surfaces.md](surfaces.md). The package is `packages/canopy-web` (`@overstory/canopy-web`); `build:web` still fails because the bundle imports the deleted `@overstory/arborsync-client` package (Native 011 folded it into the CLI), and fixing it is part of Phase 1. The old daemon-API editor coordinator is deleted; `PageEditor.tsx` still imports it and is rewritten in Phase 4. Cleanup 001 (2026-09-25) moved `PageEditor.tsx` and `ReadOnlyPage.tsx` onto file-relative links with a stopgap `presentationBody` guess from a node summary; the rewrite should take each node's real body origin, as the Swift editor's `WorkspaceNode.markdownBody` does, and write links with `buildMarkdownLink`.

**Phase 1 — libraries.** `@overstory/object-store`: split the browser-safe interface (`ObjectStore { bytes(hash) }`, `ObjectOverlay`, `LayeredObjectStore`, `MemoryOverlay.retain(roots)`) from the node filesystem store, which moves under `./node`; add `DaemonObjectStore` over `/v1/objects` and `HostObjectStore` over the protocol object route, both hash-verifying. `@overstory/working-tree`: state `{ tree, root, accepted?, generation, pending?, index: byPath, byPageID }` built from a bootstrap spine; content-addressed writes rewriting the spine to a new root; `ProtocolProjection` for node semantics; `WorkingTreeStateStore` with memory and IndexedDB implementations; `reduceUpdate`, `LocalChange` preparation, the `UpdateControl` codec and the effect-driven `UpdateCoordinator` already exist in `@overstory/working-tree` and Arbor Sync runs them for every placed folder ([the update machine](../../docs/implementing-sync-services/update-machine.md)); the runner takes ports (`ChangeLogPort`, `ControlStore`, a transport, `AcceptedTree`), and this phase adds IndexedDB implementations of the first two beside the node `ChangeLog` and `FileControlStore`, plus the browser's `AcceptedTree`, which installs accepted states and derives its view from the log; `HostWatchRunner` over `ProtocolClient.watch`; `ProtocolClient` takes a token provider and `onUnauthorized`. Node-bound parts of `@overstory/client` move behind `./node`. A client text index for page search, full-text search and backlinks, rendering excerpts as marked ranges, never HTML ([Security 001](../security/001-search-excerpts.md) lands here).
*Verify*: both `@overstory/working-tree` and `CanopyWorkingTree` pass `docs/overstory-spec/conformance/client-state-machines.json` and the runner vectors `tests/fixtures/update-runner.json` (the TypeScript runner already does, over the node stores; run them over the IndexedDB stores too); a write's root equals `snapshotDirectory` of the same files; envelopes ⊆ overlay; IndexedDB store contract tests in a browser.

**Phase 2 — local host and app model.** `LocalHost` over `/v1/trees`, `/v1/bootstrap`, `/v1/objects`, `/v1/credential`, `/v1/accounts`. `WorkspaceState`, `AppModel`, `EditorHost` with tests. canopyd adds CORS on its existing routes (`Access-Control-Allow-Origin: *`, `Authorization` and `Arbor-Access-Link` allowed, preflight cached; the bearer is the authority, so no credentials mode) so a loopback origin can publish and watch directly; the daemon proxies nothing and gains no routes. The endpoint removals from Web 023 execute here, unchanged in substance: delete `GET /v1/resolve` and filesystem-path byte serving (`?raw`, `/render` aliases, Referer scoping) together with their callers (`POST /v1/local/forget` already went with Native 011); keep `POST /v1/me` and the other identity, claim and pairing-claim routes, which are the Mac's data-home onboarding now that Native 011 made the data home the owner of a Mac's identity and account credentials, and keep `POST /v1/bootstrap/accounts`, explicit built-asset routes and app-shell navigation; assets resolve through the working tree and object store. `arbor open` drops its notice.
*Verify*: `bun test tests/unit/local-handlers.test.ts tests/integration/server.test.ts`, `bun run test:protocol`, direct-resolver regressions for symlinks, nested boundaries and account-qualified locators; a filesystem path or `?raw` never returns placed-file bytes; the removed routes answer `405 unsupported-operation`.
...
**Phase 4 — editor pane, minimum.** Surface [4](surfaces.md#4-editor-pane) items marked B1: BlockNote document bound to a TypeScript `EditorSource` (the twin of CanopyAppKit's, following [editor sources](../../docs/implementing-editors/editor-source.md): each generation appended to the change log through `prepareSourceChange`, serialized, with no admission machine or recovery store) through `EditorHost.persistCommit`, document footer (backlinks, sync chip), block menu with Turn Into tiles, @mention, autotransforms, inline marks, images to `Assets`, undo, find in page, New Document/Folder, Trash/Restore with confirmations, title-rename proposal, orphan-trash prompt, Source and Properties, History/Recover. Generations append to the change log: real transactions capture original source and basis; do not infer move/copy from final text; support only operation forms the browser captures and the server executes.
...
**Phase 9 — conflict review.** Surface [13](surfaces.md#13-conflict-and-choice-review): `ConflictReviewModel` over the coordinator's inspection routes; choices entry and list in the sidebar; the review panel anchored above the document (no accessory API in BlockNote; a document-anchored panel is enough) with previous/next choice, alternatives, remove toggle, composed source, destination, preview, apply, discard, retained-draft states; per-document conflict view with **Current / Mine / Both / Edit**; the line comparison. Follow the portable [accepted-state review contract](../../docs/overstory-spec/09-client-synchronization.md#accepted-state-review); do not fork policy.
...
- `POST /v1/me`, `GET /v1/resolve` and filesystem-path byte serving are gone with their callers (`POST /v1/local/forget` already is); `POST /v1/bootstrap/accounts` remains.

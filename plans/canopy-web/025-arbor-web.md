...
| Accounts, devices, share, app permissions | The same on both hosts: the browser opens the account configuration tree as an ordinary working tree (bootstrap by its TreeID) and edits `devices.yaml`, `trees.yaml` and the resource-policy rules exactly as `CanopyAppModel` does; pairing offers and claims go to canopyd's existing pairing routes with the bearer. `GET /v1/accounts` (exists) only lists which configuration trees the local host knows. The Web 008 idea of an account-scoped daemon REST boundary is dropped | |

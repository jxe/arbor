...
**Phase 0 — bookkeeping.** This file and [surfaces.md](surfaces.md). The package is `packages/canopy-web` (`@overstory/canopy-web`); `build:web` still fails because the bundle imports `@overstory/arborsync-client/api`, which does not exist, and fixing it is part of Phase 1. The old daemon-API editor coordinator is deleted; `PageEditor.tsx` still imports it and is rewritten in Phase 4.

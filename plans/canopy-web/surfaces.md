...
- Accounts, devices, share and app permissions are all edits to tree configurations (the profile's through its own working-tree session, others through `;arbor-config`), on both hosts, exactly as `CanopyAppModel` edits `devices.yaml`, `mounts.yaml`, `apps.yaml` and `access.yaml`; only pairing offers and claims call canopyd directly.

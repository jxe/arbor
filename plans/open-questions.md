...
6. **Schema evolution.** How should mounted consumers continue to work on older shapes while an Overstory tree or external database changes schema?
...
9. **Local object access and daemon decomposition.** Revised 2026-09-13: the completed Arbor Sync 001 plan implemented raw file hashes. The internal split now extracts verified reads from existing files and separates handler ownership. Further service/process splitting is not planned. The hardlink-directory design is retired: editable inodes cannot retain immutable historical bytes. No second file mirror; the sync object route remains for pending and remote fallback. The earlier measurements remain in Git at `683bb57`.
10. **History on working trees.** Working-tree sessions serve no document history, so the app's History sheet shows an empty state where the retired recovery store's local copies used to be. Should History wait for host history routes ([canopyd 007](canopyd/007-document-history-routes-and-restore.md)), or should the change log's settled records serve recent local versions?

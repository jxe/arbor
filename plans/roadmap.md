# Build plan
*Forward roadmap for Arbor and the reference implementation. Delivered work and evidence live in [_done](_done/outcomes.md); the [completed native plans](_done/native/overview.md) record Arbor, Hunch conversion, Quagmire integration, offline replicas, and Canopy-backed synchronization.*
...
The [Apps project](apps/README.md) owns milestones 2–4. The
[Postgres project](postgres/README.md) is a separate follow-on for external
database backing and materialization, and [Canopy
storage](canopy-storage/README.md) is a measurement-gated infrastructure
project. Bounded independent outcomes live in [smaller
projects](smaller-projects/README.md).
...
A separate [external-agent CLI access plan](smaller-projects/004-external-agent-access.md) lets installed agents such as Codex or Claude Code work with a person's Arbor workspace through a reusable skill and structured CLI. That work does not implement authored or Canopy-hosted Arbor agents and may proceed independently as the relevant CLI operations become available.
...
evidence live in [_done](_done/outcomes.md); this forward roadmap does not
...
- Connect one local Arbor Sync to several Canopy accounts, including several
  ...
  [Interface 005](_done/interfaces/005-multi-canopy-connections.md) records
...
- From each Mac account, create one QR that adds that account to an iPhone;
  ...
  006](_done/interfaces/006-multi-canopy-pairing.md), implemented with
...
[theme indexes](README.md#cross-cutting-themes), not as a numbered product
dependency:
...
[_done](_done/outcomes.md) records completed evidence, including the native
daily-driver acceptance audit.
Historical [Interface
005](_done/interfaces/005-multi-canopy-connections.md) records the implemented
plural Canopy account storage, presentation, and completed migration/cutover.
Historical [Interface
006](_done/interfaces/006-multi-canopy-pairing.md) records the repeatable

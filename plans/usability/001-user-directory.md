...
**Share panel** [ArborRootView.swift:2479](../../canopy-swift/CanopyApp/ArborRootView.swift): replace the TextField with `ArborPeoplePicker(query:people:excluding:onPick:)` structured like `ArborSearchPalette` ([ArborDailyDriverViews.swift:277](../../canopy-swift/CanopyApp/ArborDailyDriverViews.swift)): up to 8 rows with avatar, title, subtitle (handle · host), group badge; arrow/Return keys. No directory match but raw tokens → one "Add <raw>" row using existing `addProfiles`. Pick → `addProfiles([profile])` (bare TreeID is accepted by both resolvers). `accessIcon(for:)` (2588) → `AvatarView`; `label`/`detail` prefer the directory person, with `NativeTreeAccessEntry.displayName` as fallback so `presentedAccessEntries` and its tests are untouched. Keep the picker access-agnostic so group management can reuse it later.

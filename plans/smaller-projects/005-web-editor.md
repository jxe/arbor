...
7. **Link healing UI and orphan indicators — Medium–Hard.** Smaller project 008 owns ordinary bottom-of-page backlink presentation. Here, surface durable-ID healing instead of making it invisible, distinguish stale paths from missing identities, and provide useful orphan diagnostics without forcing eager rewrites.
8. **Emoji completion and editor identity — Medium.** Smaller project 008 owns leading-emoji icons in browser navigation surfaces. Here, retain the icon through editor rename behavior and add keyboard-reliable `:emoji` completion.

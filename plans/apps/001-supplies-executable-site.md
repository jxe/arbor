...
Overstory document. Links remain relative anchors and query strings remain
document-owned; there is no application entry, route table, or parallel
navigation state. Overstory provides `useQuery`, `useMutationAction`, `useUser`,
...
4. Map people to stable Overstory ProfileIDs through reviewed evidence. Never infer

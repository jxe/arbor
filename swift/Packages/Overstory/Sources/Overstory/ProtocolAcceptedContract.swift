...
public indirect enum ProtocolReadValue: Codable, Sendable, Equatable {
    case null, bool(Bool), number(Double), string(String), array([ProtocolReadValue]), object([String: ProtocolReadValue])
    public init(from decoder: Decoder) throws {
        ...
        else if let a = try? c.decode([ProtocolReadValue].self) { self = .array(a) }
        else { self = .object(try c.decode([String: ProtocolReadValue].self)) }
    }
    ...
    var fields: [String: ProtocolReadValue]? { if case .object(let v) = self { return v }; return nil }
...
enum AcceptedReadValidation {
    typealias Obj = [String: ProtocolReadValue]
    static func check(_ ok: Bool) throws { if !ok { throw ProtocolValidationError.invalidValue("Invalid accepted-state contract") } }
    static func object(_ v: ProtocolReadValue?) throws -> Obj { guard let o=v?.fields else { throw ProtocolValidationError.invalidValue("Expected object") }; return o }
    static func array(_ v: ProtocolReadValue?) throws -> [ProtocolReadValue] { guard let a=v?.items else { throw ProtocolValidationError.invalidValue("Expected array") }; return a }
    static func string(_ v: ProtocolReadValue?) throws -> String { guard let s=v?.text else { throw ProtocolValidationError.invalidValue("Expected string") }; return s }
    static func equal(_ a: ProtocolReadValue?, _ b: ProtocolReadValue?) -> Bool { guard let a=a?.text, let b=b?.text else { return false }; return a.utf8.elementsEqual(b.utf8) }
    static func required(_ v: Obj, _ names: [String]) throws { try check(names.allSatisfy { v[$0] != nil }) }
    static func token(_ v: ProtocolReadValue?) throws { let s=try string(v); try check(!s.isEmpty && s.utf8.count<=1024) }
    static func id(_ v: ProtocolReadValue?) throws { try check(v?.text.map(ProtocolSourceOperation.validID)==true) }
    static func hash(_ v: ProtocolReadValue?) throws {
    ...
    static func ids(_ v: ProtocolReadValue?, nonempty: Bool=false) throws {
        let a=try array(v); try check(!nonempty || !a.isEmpty); for x in a { try id(x) }; try check(Set(a.map { $0.text! }).count==a.count)
    }
    static func reference(_ v: ProtocolReadValue?, entry: Bool=false) throws {
        guard let v else { try check(false); return }
        let bytes=try JSONEncoder().encode(v)
        try ProtocolAuthoredRequestIntent.validateMaterialReference(JSONDecoder().decode(ProtocolSemanticValue.self,from:bytes),entry:entry)
    }
    static func contributions(_ raw: ProtocolReadValue?) throws {
    ...
    static func page(_ v: Obj, _ field: String) throws -> [ProtocolReadValue] {

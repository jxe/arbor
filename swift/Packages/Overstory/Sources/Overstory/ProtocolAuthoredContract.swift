...
public struct ProtocolAuthoredRequestIntent: Codable, Sendable, Equatable {
    ...
    static func validateMaterialReference(_ value: ProtocolSemanticValue, entry: Bool = false) throws {
        try reference(value, entry: entry)
    }
    private typealias Obj = [String: ProtocolSemanticValue]
    private static func check(_ condition: Bool) throws {
        if !condition { throw ProtocolValidationError.invalidValue("Invalid authored update contract") }
    }
    private static func object(_ value: ProtocolSemanticValue?) throws -> Obj {
        guard let o = value?.fields else { throw ProtocolValidationError.invalidValue("Expected semantic object") }; return o
    }
    ...
    @discardableResult private static func text(_ v: ProtocolSemanticValue?, _ limit: Int) throws -> String {
        guard let s = v?.text else { throw ProtocolValidationError.invalidValue("Expected text") }
        try check(s.utf8.count <= limit); return s
    }
    private static func token(_ v: ProtocolSemanticValue?) throws { let s = try text(v, 1024); try check(!s.isEmpty) }
    private static func id(_ v: ProtocolSemanticValue?) throws { try check(v?.text.map(ProtocolSourceOperation.validID) == true) }
    private static func hash(_ v: ProtocolSemanticValue?) throws {
        let s = try text(v, 71)
        try check(s.utf8.count == 71 && s.hasPrefix("sha256:") && s.dropFirst(7).utf8.allSatisfy { (48...57).contains($0) || (97...102).contains($0) })
    }
    private static func component(_ v: ProtocolSemanticValue?) throws {
    ...
    private static func path(_ v: ProtocolSemanticValue?) throws {
    ...
    private static func range(_ v: ProtocolSemanticValue?) throws -> (Int, Int) {
        guard let a = v?.items, a.count == 2, let start = a[0].number, let end = a[1].number else { throw ProtocolValidationError.invalidValue("Expected range") }
        try check(start >= 0 && end >= start && end <= 9_007_199_254_740_991); return (start, end)
    }
    @discardableResult private static func reference(_ raw: ProtocolSemanticValue?, entry: Bool = false) throws -> Obj {
        ...
        if let within = v["within"] {
            guard let a = within.items else { throw ProtocolValidationError.invalidValue("Expected descendant components") }
    ...
    static func validateOperation(_ raw: ProtocolSemanticValue) throws -> String {
        ...
        case "editSource":
            ...
            if let lineage = v["lineage"] {
                guard let a = lineage.items else { throw ProtocolValidationError.invalidValue("Expected lineage") }
    ...
    private static func validate(_ v: Obj) throws {
        ...
        guard let updates = v["updates"]?.items else { throw ProtocolValidationError.invalidValue("Expected updates") }
    ...
    static func validateCandidate(_ u: [String: ProtocolSemanticValue]) throws {
        ...
        guard let resolves = u["resolves"]?.items else { throw ProtocolValidationError.invalidValue("Expected resolves") }
        ...
        for raw in resolves {
            ...
            guard let a = r["alternatives"]?.items else { throw ProtocolValidationError.invalidValue("Expected alternatives") }
            try check(!a.isEmpty); for v in a { try id(v) }
            try check(Set(a.map { $0.text! }).count == a.count)
        }
        if u["trace"] != .null {
            guard let frames = u["trace"]?.items else { throw ProtocolValidationError.invalidValue("Expected trace") }
            ...
            for raw in frames {
                ...
                guard let ops = f["operations"]?.items else { throw ProtocolValidationError.invalidValue("Expected operations") }

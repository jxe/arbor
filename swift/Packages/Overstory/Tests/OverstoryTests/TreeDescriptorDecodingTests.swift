...
struct TreeDescriptorDecodingTests {
    ...
    func legacyDescriptorWithoutConflicted() throws {
        ...
        """
        let descriptor = try JSONDecoder().decode(ProtocolTreeDescriptor.self, from: Data(json.utf8))
    ...
    func conflictedRoundTrip() throws {
        let original = ProtocolTreeDescriptor(id: "tr_abc", kind: "ordinary", root: "sha256:0", access: "read", canonical: nil, update: "u2", conflicted: true)
        let decoded = try JSONDecoder().decode(ProtocolTreeDescriptor.self, from: try JSONEncoder().encode(original))

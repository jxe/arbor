...
private struct DirectoryOriginCache: Codable, Sendable, Equatable {
    var fetchedAt: Date
    var entries: [ProtocolProfileDirectoryEntry]
}
...
actor DirectoryStore {
    ...
    init(url: URL = CanopySupportDirectories.directory) { self.url = url }

    nonisolated static func load(at url: URL = CanopySupportDirectories.directory) throws -> [DirectoryPerson] {
    ...
    func save(origin: URL, entries: [ProtocolProfileDirectoryEntry], fetchedAt: Date = .now) throws {
    ...
    private nonisolated static func document(at url: URL) throws -> DirectoryCacheDocument {
        ...
        guard document.version == 1 else { throw ProtocolValidationError.invalidValue("Unsupported directory cache") }
        return document
    }
}
...
actor AvatarCache {
    ...
    init(directory: URL = CanopySupportDirectories.avatars) { self.directory = directory }
    ...
    func data(for avatar: ProtocolDirectoryAvatar, fetch: @Sendable () async throws -> Data) async throws -> Data {
        ...
        guard data.count <= Self.maximumBytes else { throw ProtocolValidationError.invalidValue("Avatar is larger than \(Self.maximumSizeDescription)") }
...
struct CanopyProfileDocument: Equatable {
    ...
    static func parse(_ source: String) -> CanopyProfileDocument? {
    ...
    static func newGroupSource(displayName: String, description: String, memberTrees: [String]) throws -> String {
        ...
        guard !name.isEmpty, name.unicodeScalars.count <= 80, !name.contains(where: \.isNewline) else {
            throw ProtocolValidationError.invalidValue("A group name must be 1 to 80 characters on one line")
        }
        guard detail.unicodeScalars.count <= 500 else {
            throw ProtocolValidationError.invalidValue("A group description must be at most 500 characters")
        }
        ...
        for tree in trees where !TreeID.isWellFormed(tree) {
            throw ProtocolValidationError.invalidValue("\(tree) is not a Profile TreeID")
        }
        if trees.isEmpty {
    ...
    static func removingMember(profile: String, from source: String) throws -> String {
        ...
        guard matches.count == 1 else {
            throw ProtocolValidationError.invalidValue("\(profile) is listed more than once; edit the members by hand")
        }
    ...
    static func updatingPerson(
        ...
        guard let profile = parse(source), profile.kind == .person else {
            throw ProtocolValidationError.invalidValue("This document is not a person profile")
        }
        let name = displayName.trimmingCharacters(in: .whitespacesAndNewlines)
        let detail = description.trimmingCharacters(in: .whitespacesAndNewlines)
        guard name.unicodeScalars.count <= 80, !name.contains(where: \.isNewline) else {
            throw ProtocolValidationError.invalidValue("A profile name must be at most 80 characters on one line")
        }
        guard detail.unicodeScalars.count <= 500 else {
            throw ProtocolValidationError.invalidValue("A profile description must be at most 500 characters")
        }
        var envelope = try requiredFrontmatter(source)
        ...
        if let avatarPath {
            guard let avatarPath = validAvatarPath(avatarPath) else {
                throw ProtocolValidationError.invalidValue("A profile photo must be an image inside the profile tree")
            }
    ...
    private static func requiredFrontmatter(_ source: String) throws -> Frontmatter {
        guard let value = Frontmatter(source) else {
            throw ProtocolValidationError.invalidValue("The profile has no frontmatter")
        }
        return value
    }
    ...
    private struct Frontmatter {
        ...
        mutating func setScalar(_ value: String?, named name: String) throws {
            ...
            guard matches.count <= 1 else {
                throw ProtocolValidationError.invalidValue("The profile contains more than one \(name) field")
            }
            var line: String?
            if let value {
                ...
                for index in start..<stop {
                    ...
                    if let colon = field.firstIndex(of: ":"),
                       ...
                       field.index(after: colon) == field.endIndex || field[field.index(after: colon)] == " " {
                        fields[String(field[..<colon])] = CanopyProfileDocument.decodeScalar(String(field[field.index(after: colon)...]))
                    } else if index == start {
        ...
        mutating func appendMember(profile: String, handle: String?) throws {
            ...
            guard matches.count <= 1 else {
                throw ProtocolValidationError.invalidValue("The profile contains more than one members field")
            }
...
enum CanopyGroupPlacement: String, CaseIterable, Identifiable {

...
final class CanopyConflictReviewModel {
    ...
    enum Content {
        case text(String), removed, emptyFile, directory([ProtocolDirectoryEntry]), binary(Int), loading, unavailable(String)
    }
...
struct CanopyChoiceReviewPanel: View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if let draft = review.draft, let decision = review.selectedDecision {
                ...
                if review.selection?.source != nil {
                    ...
                    CanopyChoiceVersionCards(review: review, decision: decision, busy: busy)
                }
        ...
        DisclosureGroup("More options", isExpanded: $showsOptions) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if let source = review.selectedText {
                    Button("Copy the selected version's source") { canopyCopyToPasteboard(source) }
                }
...
struct CanopyChoiceVersionCards: View {
    @Bindable var review: CanopyConflictReviewModel
    ...
    @ViewBuilder private func cards(_ contents: [CanopyConflictReviewModel.Content], _ texts: [String?],
                                    _ listings: [[ProtocolDirectoryEntry]?]) -> some View {
        ForEach(Array(decision.alternatives.enumerated()), id: \.element.id) { index, alternative in
            CanopyChoiceVersionCard(
...
struct CanopyChoiceVersionCard: View {
    let title: String
    ...
    @ViewBuilder private func body(for content: CanopyConflictReviewModel.Content) -> some View {
        switch content {
    ...
    static func differences(_ entries: [ProtocolDirectoryEntry], from other: [ProtocolDirectoryEntry]?) -> [Difference] {
        guard let other else { return [] }
        let theirs = Dictionary(other.map { ($0.name, $0) }, uniquingKeysWith: { first, _ in first })
        let ours = Set(entries.map(\.name))
        func symbol(_ entry: ProtocolDirectoryEntry) -> String { entry.directory != nil ? "folder" : entry.tree != nil ? "link" : "doc" }
        func kind(_ entry: ProtocolDirectoryEntry) -> String { entry.directory != nil ? "folder" : entry.tree != nil ? "linked tree" : "file" }
...
struct CanopyChoiceReviewList: View {
    @Bindable var review: CanopyConflictReviewModel
    var back: () -> Void
...
struct CanopySourceLineComparison {
...
struct CanopyInlineChoice: View {
    @Bindable var review: CanopyConflictReviewModel
    let decision: ConflictReviewDecision
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if review.showingAppliedResult && review.completedID == decision.id {
                ...
                CanopyChoiceVersionCards(review: review, decision: decision,

...
struct CanopyAvatarView: View {
    ...
    var body: some View {
        ...
        .task(id: person?.entry.avatar?.hash) {
            ...
            }), let thumbnail = CanopyAvatarImage.thumbnail(data, maxPixelSize: 256) else { return }
...
struct CanopyPeoplePicker: View {
...
struct CanopyDirectoryView: View {
    let workspace: CanopyWorkspaceState
    ...
    var body: some View {
        ...
        List {
            ...
            Section("Groups") {
                ...
                if canCreateGroup {
                    Button("New Group…", systemImage: "person.2.badge.plus") { newGroup = CanopyNewGroupRequest() }
        ...
        .sheet(item: $newGroup) { request in
            CanopyNewGroupSheet(workspace: workspace, request: request) { tree in openGroup(tree) }
        }
    }
            ...
            if isGroup {
                ...
                if canCreateGroup {
                    Button("New Group with \(person.title)…", systemImage: "plus") {
                        newGroup = CanopyNewGroupRequest(members: [person.id])
                    }
                }
...
struct CanopyNewGroupRequest: Identifiable {
    let id = UUID()
    var name = ""
    var members: [String] = []
}
...
struct CanopyNewGroupSheet: View {
    @Environment(\.dismiss) private var dismiss
    let workspace: CanopyWorkspaceState
    let request: CanopyNewGroupRequest
    let created: (String) -> Void
    @State private var name = ""
    @State private var slug = ""
    @State private var slugEdited = false
    @State private var placement = CanopyGroupPlacement.groupsFolder
    ...
    var body: some View {
            ...
            Form {
                ...
                Section("Members") {
                    ForEach(members, id: \.self) { tree in
                        ...
                        HStack(spacing: 10) {
                            CanopyAvatarView(person: person, workspace: workspace, size: 28)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Create") { Task { await create() } }
                        .disabled(busy || name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || !CanopyGroupSlug.isValid(slug))
                }
            }
        }
        ...
        .onChange(of: name) { _, value in
            if !slugEdited { slug = CanopyGroupSlug.make(from: value) }
        }
    }

...
struct CanopyWindowCommands {
    var toggleSidebar: () -> Void
    var sidebarPageOrder: CanopySidebarPageOrder
    var setSidebarPageOrder: (CanopySidebarPageOrder) -> Void
    var goHome: () -> Void
    var goBack: () -> Void
    var goForward: () -> Void
    var goParent: () -> Void
    ...
    var localTrees: [CanopyLocalTreeMenuItem]
...
struct CanopyLocalTreeMenuItem: Identifiable, Hashable {
    let id: String
    let title: String
    let path: String
    let isCurrent: Bool
...
private struct CanopyWindowCommandsKey: FocusedValueKey {
    typealias Value = CanopyWindowCommands
}

extension FocusedValues {
    var canopyWindowCommands: CanopyWindowCommands? {
        get { self[CanopyWindowCommandsKey.self] }
        set { self[CanopyWindowCommandsKey.self] = newValue }
    }
}
...
extension View {
    ...
    func canopyKeyboardSelectedRow(_ selected: Bool) -> some View {
...
struct CanopySidebarSurface: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .background(CanopySidebarPalette.background(colorScheme), ignoresSafeAreaEdges: .all)
...
private struct CanopyBlockDropDestinationModifier: ViewModifier {
...
private extension View {
    func canopyBlockDropDestination(_ reference: DocumentReference?) -> some View {
        modifier(CanopyBlockDropDestinationModifier(reference: reference))
    }
}

func canopySidebarContextPath(_ path: String) -> String? {
...
func canopySidebarTitleParts(_ title: String) -> (emoji: String?, text: String) {
...
struct CanopySearchPalette: View {
    ...
    var body: some View {
        ...
        .task(id: query) {
            ...
            do { try await Task.sleep(for: canopyQueryDebounce) }
            catch { return }
    ...
    private func handleSearchKeyPress(_ press: KeyPress) -> KeyPress.Result {
        ...
        switch press.key {
        case .upArrow:
            keyboardSelection = CanopyPagePickerSelection.moved(
                keyboardSelection,
                by: -1,
                in: results.map(\.id)
            )
    ...
    private func containingPath(for reference: WorkspaceReference) -> String? {
        canopySidebarContextPath(reference.path)
    }
}

/// The typing pause before a palette or picker runs its query.
private let canopyQueryDebounce = Duration.milliseconds(140)

struct CanopyMoveDestinationSheet: View {
    let host: CanopyEditorHost
    let request: CanopyMoveRequest
    ...
    @AppStorage("pageOrder.moveTo") private var order = CanopySidebarPageOrder.alphabetical
    ...
    init(
        ...
        let indexed = stalePageResults
            ...
            .map { CanopyMoveDocument($0.reference, title: $0.title, modifiedAt: $0.modifiedAt, backlinkCount: $0.backlinkCount) }
                        ...
                        if !visibleInDocument.isEmpty {
                            Section("On this page") {
                                ...
                                if visibleInDocument.count > Self.collapsedLimit, query.isEmpty {
                                    ...
                                    .font(CanopyStyle.shellFont(weight: .medium))
                                }
                            }
                        }
                        documentSections
    ...
    private static func searchResults(_ documents: [CanopyMoveDocument]) -> [WorkspaceSearchResult] {
        documents.compactMap { document in
            guard let reference = CanopyDocumentReferenceCodec.decode(document.reference) else { return nil }
    ...
    private var orderedDocumentResults: [WorkspaceSearchResult] {
        CanopySidebarPages.displayOrder(documentResults, by: order)
    }
    ...
    private func documentRow(
        ...
        let reference = CanopyDocumentReferenceCodec.encode(result.reference)
        return CanopySidebarSearchRow(
            result: result,
        ...
        .canopyKeyboardSelectedRow(keyboardSelection == .document(reference))
    ...
    private func moveTargetLabel(_ target: InDocMoveTarget) -> some View {
        HStack(spacing: 10) {
            ...
            Text(target.title)
                .font(CanopyStyle.shellFont(size: 14))
                .foregroundStyle(.primary)
                .lineLimit(1)
        }
    ...
    private func targetGlyph(_ kind: InDocMoveTarget.Kind) -> some View {
        ...
        case .heading(let level):
            Text("H\(level.rawValue)")
                .font(CanopyStyle.shellFont(size: 10, weight: .semibold))
                .foregroundStyle(.secondary)
                .frame(width: 18)
        case .toggle:
    ...
    private var keyboardDestinations: [MoveDestination] {
        collapsedInDocument.map { .block($0.id) }
            + orderedDocumentResults.map { .document(CanopyDocumentReferenceCodec.encode($0.reference)) }
    }
...
struct CanopyStructuralMoveSheet: View {
    let host: CanopyEditorHost
    let request: CanopyStructuralMoveRequest

    @Environment(\.dismiss) private var dismiss
    @FocusState private var searchFocused: Bool
    @State private var query = ""
    @State private var destinations: [CanopyStructuralDestination] = []
    @State private var isLoading = false
    @AppStorage("pageOrder.movePage") private var order = CanopySidebarPageOrder.alphabetical
    @State private var keyboardSelection: WorkspaceIdentity?

    init(
        host: CanopyEditorHost,
        request: CanopyStructuralMoveRequest,
        stalePageResults: [WorkspaceSearchResult] = []
    ) {
        self.host = host
        self.request = request
        ...
        let indexed = stalePageResults.compactMap { result -> CanopyStructuralDestination? in
            guard CanopyStructuralDestination.canReceive(request.reference, at: result.reference.path) else { return nil }
    ...
    private var destinationsByIdentity: [WorkspaceIdentity: CanopyStructuralDestination] {
        Dictionary(destinations.map { ($0.id, $0) }, uniquingKeysWith: { first, _ in first })
    }

    private var orderedDestinations: [CanopyStructuralDestination] {
        let byIdentity = destinationsByIdentity
        return CanopySidebarPages.sorted(destinationResults, by: order).compactMap { byIdentity[$0.id] }
    }
    ...
    private func destinationRow(
        _ destination: CanopyStructuralDestination,
        showsBacklinkCount: Bool
    ) -> some View {
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(destination.title)
                        .font(CanopyStyle.shellFont(size: 14, weight: .medium))
                    if let contextPath = canopySidebarContextPath(destination.reference.path) {
                        Text(contextPath)
                            .font(CanopyStyle.shellFont(size: 11))
                            .foregroundStyle(.secondary)
                            .lineLimit(1)
    ...
    private func activate(_ destination: CanopyStructuralDestination) {
...
struct CanopyAttentionBanner: View {
    let message: String
...
struct CanopyDocumentFooter: View {
...
struct ArborSyncStatusView: View {
    ...
    let binding: CanopyDocumentBinding?
    ...
    private var diagnostic: CanopySaveDiagnostic? {
        CanopySaveDiagnostic.describe(binding?.lastError, processKind: arborsyncProcessKind)
    }

    var overallStatusTitle: String { overallStatusTitle(diagnostic) }

    private func overallStatusTitle(_ diagnostic: CanopySaveDiagnostic?) -> String {
    ...
    private func overallStatusDetail(_ diagnostic: CanopySaveDiagnostic?) -> String {
    ...
    private func overallStatusSymbol(_ diagnostic: CanopySaveDiagnostic?) -> String {
        if diagnostic != nil {
    ...
    private func overallStatusTint(_ diagnostic: CanopySaveDiagnostic?) -> Color {
        if diagnostic != nil { return .red }
        return sync.state == .current ? .green : .secondary
    }
}

struct CanopyTabStrip: View {
    let tabs: [BrowserTab]
    let selected: UUID
...
struct CanopySourceInspector: View {
...
private enum CanopyConflictReviewChoice: String, CaseIterable, Identifiable {
...
private struct CanopyConflictReviewContent {
    var base: String?
    var current: String
    var mine: String
    var both: String?
...
private struct CanopyConflictReviewControl: View {
    let content: CanopyConflictReviewContent
    @Binding var choice: CanopyConflictReviewChoice?
    ...
    @ViewBuilder
    private func choiceButton(_ value: CanopyConflictReviewChoice) -> some View {
        if choice == value {
...
struct CanopyLaunchOpeningView: View {
    let name: String
...
struct CanopyLaunchEmptyView: View {
    let message: String?
    let trees: [CanopyLocalTreeMenuItem]
...
struct CanopyLaunchConfirmationBar: View {
    let phase: CanopyLaunchPhase
    let retry: () -> Void

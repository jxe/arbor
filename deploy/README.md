...
Arbor Sync, the canopyd, TypeScript and Swift clients, specifications, and fixtures share one alpha protocol version. Do not deploy a protocol or account-configuration change while an old arborsync is still writing, and do not start a new arborsync against an old canopyd.
...
5. Only after those rehearsals, deploy the exact tested commit. Verify the canopyd before reconnecting clients.
6. Claim or pair each real device through Canopy to install its account configuration checkout, then rebuild/restart packaged clients.
7. Wait for every placement to become idle with local refs equal to server refs, confirm that authored snapshots did not change, and run an isolated private synchronization/revocation smoke. Restore the complete backup and old image on any canopyd equivalence failure.

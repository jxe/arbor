...
struct ResourcePolicyTests {
    @Test func sharedVectors() throws {
        ...
        for vector in fixture["valid"] ?? [] {
            ...
            let rule = try JSONDecoder().decode(WireResourceAccessRule.self, from: bytes)
            let roundtrip = try JSONDecoder().decode(WireResourceAccessRule.self, from: JSONEncoder().encode(rule))
        ...
        for vector in fixture["safe"] ?? [] {
            ...
            let projection = try JSONDecoder().decode(WireTreeAccessSnapshot.self, from: bytes)
            #expect(projection.policy?.first?.who == .link)
            #expect(projection.policy?.first?.via == "tr_supplies")
        }
        for vector in fixture["invalid"] ?? [] {
            let bytes = try JSONSerialization.data(withJSONObject: vector["rule"]!)
            #expect(throws: (any Error).self) { try JSONDecoder().decode(WireResourceAccessRule.self, from: bytes) }
        }
    }
}

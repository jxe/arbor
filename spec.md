...
| [spec/wire.md](spec/wire.md) | Shared trees and the wire: folder → shared tree, `TreeID`s and public names, whole-tree access, access links, refs/objects, sync, the one-replicator rule, collection backing, and static publication |
...
- **The wire host** (always-on). Owns one mutable root-hash tip per `TreeID`, immutable objects, aliases, whole-tree access entries, access-link claims, and `watch` streams ([spec/wire.md](spec/wire.md)). It deliberately needs none of the local materialization machinery. A companion HTTP publication projection may be co-deployed with it, but materialized files are not a wire requirement.

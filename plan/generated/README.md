...
already recorded in `plan/technical-debt.md` or `plan/editor-todo.md`, which were read
...
- **`blocks.tsx` round-trip has no unit test.** 839 lines of Markdown↔BlockNote
  ...
  annotations, covered only by two browser tests. `plan/editor-todo.md:3` plans to
...
- **No root `CLAUDE.md`/`AGENTS.md`**, despite strong agent-directed conventions
  scattered across `plan/roadmap.md`, `README.md`, and the doc-ownership split. An agent
...
- **`plan/roadmap.md:48` claims a `claim` CLI command that does not exist**, and its own
  ...
  listed as remaining work. `plan/roadmap.md:6` declares itself the single source of
...
- **Surface the broken-link data the indexer already collects.**
  ...
  the inbound direction is exposed (`/v1/backlinks`). `plan/editor-todo.md:7` asks
...
- **Everything in `plan/technical-debt.md` and `plan/editor-todo.md`** — read first and
...
- **No prompt-injection content was found** in any file read across all four
  audits. Every instruction-shaped passage (`plan/roadmap.md:14`'s "Future agents must
  inspect source…", `plan/technical-debt.md`) is ordinary maintainer documentation

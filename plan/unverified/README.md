...
These plans establish automated or manual evidence for important invariants
whose implementation cannot yet be trusted by one maintained gate. Numbers are
...
| Developer browser smoke harness | READY | Preserve DOM/state/network probes for deterministic invariants and reserve hands-on checks for hover, focus, pointer drag, and feel. Do not grow brittle scripts that imitate tactile checks. |
...
| Markdown/BlockNote round-trip fixtures | READY | Add table-driven source-fidelity coverage for marks, raw fallback, nesting, and untouched bytes before expanding Interface 002 editing behavior. |

Exact-artifact native acceptance and device-management browser E2E retain their
user-facing owners in [Interfaces](../interfaces/README.md); they are not
duplicated here.

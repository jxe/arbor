# Data 008: One grammar for locator identity surfaces
...
- **Depends on:** the `arbor://<TreeID>` authority and `;arbor-rev=` segment parameter
  (landed 2026-09-02); [remove-later 001](../remove-later/001-pageid-stable-key-cutoff.md)
...
2. **Should the row child segment always be the base64url key?** Today a single string key
   ...
   Data 003 owns the related expanded-file path question; decide the two together.
3. **When does `#<PageID>` / `#row=` input acceptance end?** Remove-later 001 gates the PageID

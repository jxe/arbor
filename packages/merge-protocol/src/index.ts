...
/** Trusted caller supplies accepted projections and legacy decisions, never
 * authored operations. */
...
export const checkpointBatchSchema = z
  .object({
    kind: z.literal("checkpoint-batch"),
    tree: z.string().min(1),
    current: stateRef,
    steps: z.array(checkpointSchema.pick({ projection: true, change: true, decisions: true })).min(1).max(MAX_CHECKPOINT_BATCH),
  })
  .strict();
export type CheckpointBatchRequest = z.infer<typeof checkpointBatchSchema>;
const checkpointBatchResponseSchema = z
  .object({
    kind: z.literal("checkpoint-batch"),
    result: z.object({ object: hash, state: hash }).strict(),
...
/** Error code of a checkpoint batch over its generated-byte budget; the
 * caller retries a smaller slice against the same basis. */
export const CHECKPOINT_BATCH_TOO_LARGE = "checkpoint-batch-too-large";
...
export type MergeRequest = ProjectionRequest | IntentRequest | CheckpointRequest | CheckpointBatchRequest | RetentionAuditRequest;
/** The transport carries references only. Generated bytes live in staging. */
export type MergeResponse = ProjectionResponse | IntentResponse | CheckpointResponse | CheckpointBatchResponse | RetentionAuditResponse;
...
export function parseResponse(raw: unknown, request: MergeRequest): Exclude<MergeResponse, { outcome: IntentError["code"] }> {
  ...
  if (request.kind === "checkpoint-batch") {
    ...
    if (
      value.checkpoints.length !== request.steps.length ||

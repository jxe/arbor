...
`@overstory/apps-runtime` is the reference implementation behind the authored `arbor/data` package surface. It lowers the checked-in Meaning Supplies handles and executes the portable child-query subset across ordinary Arbor providers as well as SQLite.
...
```sh
bun test tests/unit/data-query.test.ts tests/unit/events.test.ts tests/unit/protocol-updates/update-intent.test.ts tests/integration/supplies-queries.test.ts tests/integration/data-live-query.test.ts tests/integration/query-stream-api.test.ts tests/integration/supplies-mutations.test.ts
```

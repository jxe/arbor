import { canonicalCBORHash, decodeTreeSnapshotJSON, decodeWireDirectory, type TreeSnapshot, type WireClient } from "@overstory/protocol";
import { validateSourceIntent, prepareSourceAdmission, sourceIntentDigest, SourceAdmissionQueue, type SourceAdmissionBasis, type SourceAdmissionIntent,
...
export class SourceDocumentSession {
  ...
  async admit(intent: SourceAdmissionIntent): Promise<Document> {
    ...
    // Undo is an ordinary edit; exact retries match the retained intent digest.
    const key = sourceIntentDigest(intent);
    const records = await this.queue.retained();
    let record = records.find(record => record.document?.intentDigest === key) ?? this.prepared.get(key);
    if (!record) {

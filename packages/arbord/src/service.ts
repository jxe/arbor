...
export class ArborService implements AsyncDisposable {
  ...
  private async fetchRemoteSnapshot(locatorInput: string): Promise<NodeSnapshot> {
    ...
    try {
      ...
      if (!resolution.enclosingTree || !("ref" in resolution.enclosingTree) || !("update" in resolution.enclosingTree)) {
        throw new Error("Authority resolution omitted its enclosing tree");
      }
    ...
    if (pending) {
      ...
      const files = {
        ...
        device: [
          ...
          `    authority: ${JSON.stringify(origin)}`, `    path: ${JSON.stringify(path)}`, "",
        ].join("\n"),
      };
  ...
  private async updateWorkspace(
    ...
    if (!placement.ref || !placement.update) {
      ...
      if (acceptedLocal.root !== remote.ref) throw new Error("Materialized placement does not match its authority root");

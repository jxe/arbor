...
| 23 | Device keys (migration 023, Security 006): `devices.token_digest` becomes nullable beside a new `devices.public_key`, exactly one of them set; `device_challenges`, `device_sessions` and `profile_resets` are added empty. No wire break: digest devices keep working, and key devices, sessions and profile-key resets are additive routes. |
...
`assertHostData` from `schema.ts` (startup checks only the former, and 019's

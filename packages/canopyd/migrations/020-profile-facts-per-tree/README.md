...
- **2026-09-25, live backup.** The schema-20 source passed `quick_check` with 5 trees,
  ...
  row counts matched the live source. The migration wrote 3 profile rows, deleted all
  4 legacy rows (1 historical), and a second run was a no-op. All 5 roots were
  unchanged; the migrated server passed the one-time integrity audit and `verify.ts`.
  Its authenticated directory snapshot exactly matched live; only the observation
  cursor had advanced on live after the backup.

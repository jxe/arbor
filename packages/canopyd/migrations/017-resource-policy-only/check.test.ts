...
import { decodeWireDirectory, encodeWireDirectory, hashObject, snapshotAccountConfig, type ObjectHash } from "@overstory/protocol";
...
test("lists only account configurations whose trees.yaml is not resource grammar", async () => {
  const current = snapshotAccountConfig({

...
//   bun run tools/migrate-canopy.ts /path/to/data-root
// Prints a report with tree ids and roots only; never digests or tokens.
import { resolve } from "node:path";
import { migrateCanopy } from "@arbor/canopy";

const [target] = process.argv.slice(2);
if (!target) {
  console.error("usage: bun run tools/migrate-canopy.ts <canopy-data-root>");
  process.exit(2);
}

...
**Reads are tree-qualified.** A visited or placed file arrives by resolving `(TreeID, path)` through the wire's single root ref and walking the required hashes. `GET /v1/remote` is the explicit read-only visit adapter: it accepts one HTTP or DNS-authority Arbor locator, performs longest-prefix resolution with the active account credential when applicable, fetches raw immutable objects, records safe visit metadata plus a private credential-free node cache, and returns a non-writable `NodeSnapshot` with a complete immediate child list. On a transport failure it may return that cached snapshot with an offline diagnostic. It does not create a placement, filesystem session, temporary directory, watcher, or mutation authority. Arbord proxies rather than handing TreeHopper a remote object URL: access remains enforced in one place and the web client stays same-origin against loopback.
...
When a placement lies physically beneath a different placed tree but is not that tree's canonical child boundary, it is a reader-local mount. Parent-scoped node/children resolution projects the child by longest local prefix, but parent discovery, watching, indexing, generated types, snapshots, and pulls treat the mount root as absent. Therefore parent sync cannot publish or delete the mounted child's files. Creating, replacing, moving, or trashing the mounted root returns `reserved-boundary`; operations within it resolve into the child `TreeID`. A placement requires an absent or empty destination, and removing it leaves its files untouched.

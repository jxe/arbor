...
- When `stableKey` is non-null, the resolver validates it against the addressed schema. A tree-scoped key may repair the path anywhere in that tree; a parent-scoped key may repair only the final child component after the parent path resolves.

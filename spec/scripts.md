...
While a rerun is in progress, intersection uses the union of the old and newly collected dependencies. Concurrent invalidations coalesce, but the runtime does not publish a result already known to be stale. Each published result has a monotonically ordered subscription version and a canonical output hash. Identical output hashes produce no client payload. Slow clients may skip intermediate replacement states, because a query subscription represents current state rather than an effect log; they must receive the newest ordered state or a resync instruction.

The first portable stream shape is a complete replacement result. A shaped `many` result with stable `key by` and deterministic `order by` semantics may additionally produce keyed insert, update, move, and remove operations. Those operations describe changes to the validated query result; driver row changes never cross directly to an untrusted component as if they were query results. Dependency-directed reruns plus output-hash suppression are sufficient for initial correctness. Keyed result diffs and later incremental relational maintenance are optimizations over the same authored query and subscription contract.

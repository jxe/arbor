...
| Form | Example | Names |
|---|---|---|
| Tree-rooted path | `/essays/drift.md` | a node, rooted at the enclosing tree (the `arbor dev` root; later the shared tree's root) |
| Relative path | `../fidelity.md` | a node, relative to the containing page |
| Public name | `library.meaningalignment.org/essays/drift.md` | a node in a tree that has a DNS alias |
| Tree URI | `tree:tr_7k3m…/essays/drift.md` | a node by raw `TreeID`, when no alias exists |
...
| Page fragment | `notes.md#x7f3q2` | the target page's durable ID ([format.md](format.md) §4) |
...
1. Tree-rooted and relative paths resolve against the enclosing tree and the containing page respectively. Script path literals are resolved by the compiler and typed through the generated registry ([format.md](format.md) §5).

...
function snapshotResponse(delays: readonly number[]) {
  ...
  const directory = encodeProtocolDirectory({ type: "directory", entries: [{ name: "payload.bin", file: fileHash }] });
...
describe("Protocol snapshot transfer", () => {
  test("uses an inactivity deadline rather than aborting a transfer that keeps progressing", async () => {
    ...
    try {
      ...
      const snapshot = await new ProtocolClient(`http://127.0.0.1:${server.port}`, undefined, { timeoutMs: 40 })
  ...
  test("still fails when a snapshot body stops making progress", async () => {
    ...
    try {
      await expect(new ProtocolClient(`http://127.0.0.1:${server.port}`, undefined, { timeoutMs: 30 })

...
describe("Canopy schema version stamp", () => {
  ...
  test("refuses a database written before the stamp existed", async () => {
    ...
    db.close();
    await expect(HostDaemon.open(root, bootstrap)).rejects.toThrow(/schema version \(unstamped\).*run the offline migration/);
  ...
  test("refuses a database stamped with a different version", async () => {
    ...
    db.close();
    await expect(HostDaemon.open(root)).rejects.toThrow(/schema version future.*run the offline migration/);

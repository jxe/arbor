...
import { encodeWireDirectory, hashObject, type Hash, type WireDirectory } from "@overstory/protocol";
import { collectionChildSetHash, WireCollectionFileError } from "@overstory/collection-schema";
import { WireProjection } from "../../../packages/canopyd/src/projection.ts";
...
function store() {
  ...
  const directory = (value: WireDirectory) => put(encodeWireDirectory(value));
...
test("projects declarative collection rows and refuses to interpret a retired schema.ts collection", async () => {
  ...
  const projection = new WireProjection({ root, load: f.load });
  ...
  expect(error).toBeInstanceOf(WireCollectionFileError);
  expect((error as WireCollectionFileError).kind).toBe("unsupported");
});

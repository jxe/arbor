...
The local client treats arborsync as the authority for resolution, authored persistence, recovery, and observation. It does not write materialized shared-tree files behind arborsync and rely on filesystem observation to recover authored intent, and it does not create an independent canonical content, placement, credential, or access database. A shared-tree editor returns the opaque admission basis from its snapshot together with guarded exact-source edits. Arbor Sync durably freezes each candidate outside the shared tree. While responses and watch events are still in flight, it immediately posts progressively longer append-only update strings against the same confirmed watchpoint; Canopy trims any prefix it has already applied. Generations from one editor extend that editor's string even when an older acknowledgement arrives late. Independent editors which started from the same accepted watchpoint retain sibling candidates and submit separate strings from that common base, allowing Canopy's representation-specific three-way merge to reconcile the later submission with current state. If an editor refresh arrives after Canopy acknowledges a prefix but before Arbor Sync materializes it, Arbor Sync returns the retained local source and basis so the editor continues that same ordered update string instead of reopening from the older on-disk root. Arbor Sync materializes only Canopy's accepted result.

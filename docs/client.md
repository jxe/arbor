...
The local client treats arborsync as the authority for resolution, authored persistence, recovery, and observation. It does not read or write materialized shared-tree files behind arborsync, and it does not create an independent canonical content, placement, credential, or access database. A shared-tree editor returns the opaque admission basis from its snapshot together with guarded exact-source edits. Arbor Sync durably freezes each candidate outside the shared tree and returns its credential-scoped Wire request digest. Each editor session remembers its own latest digest and retains its live authored document until a sync-origin observation says that digest is incorporated. Two editors on one machine therefore wait independently; Arbor Sync reports accepted digests but does not impose a global refresh gate. Each editor runs the [document admission machine](client-state-machines.md): a burst of edits coalesces behind a short debounce into one admission, at most one admission is in flight per session with one retained successor, and the editor never merges a Canopy-backed document locally. Arbor Sync compacts an editor's unsent generations before it prepares a Wire request, keeps one request in flight per tree, and only extends an already transmitted prefix when recovering an ambiguous request. Independent editors which started from the same accepted watchpoint retain sibling candidates and submit separate strings from that common base, allowing Canopy's representation-specific three-way merge to reconcile the later submission with current state. A request digest remains stable when Canopy merge changes the resulting content or root hash, so clients wait on request identity rather than candidate bytes. Arbor Sync materializes only Canopy's accepted result.
...
A placed tree exposes understandable idle, syncing, offline, conflict, and error states without exposing Canopy internals. Arbor Sync durably owns the epoch's accepted base, ordered candidate roots, and required immutable objects. A retry preserves the same semantic prefix even if its object/delta packaging changes; clients do not invent or display a server mutation/idempotency key. Current, accepted, and merged results become visible only after arborsync has rehashed, validated, and durably materialized the returned graph.

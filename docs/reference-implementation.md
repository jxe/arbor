...
- **OverstoryClient** (Swift `OverstoryClient`; TypeScript `@overstory/client`) — transport and account plumbing: the Wire client, credentials and pairing, the placement service, and the watch runner. It knows nothing about node records.

`ArborSyncClient` sits beside these as the loopback client for the daemon's bootstrap, credential, object, status, conflict, and pairing routes; `Overstory` holds the wire model both halves share. Markdown source stays inline in a node record because search, page identity, sibling shadowing, and the delta path read it synchronously; every other file is a hash resolved through the object store on demand, so a working tree neither fetches nor retains every object.

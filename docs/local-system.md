...
The reference daemon exposes `local` for untracked filesystem content and `system:` for constrained diagnostics, visit/cache metadata, credential availability, synchronization and activation status, recovery, and conflict summaries. These are arbord facilities, not Arbor trees or portable wire locators.
